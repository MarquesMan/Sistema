...
/**
 * Verifica os campos de um formulário
 *
 * Verifica se todos os campos passados foram enviados pelo formulario.
 * Obs.: Essa função está no escopo global, pois, vamos precisar muito da mesma.
 *
 * @param array  $fields campos enviados pelo formulário
 * @return true|false  Se contém todos os campos
 */
function chk_form_fields ( $fields ) {
	if(!is_array($fields)){
		return false;
	}
...
function __autoload($class_name) {
	...
	if ( ! file_exists( $file ) ) {
		$file = ABSPATH . '/table/' . $class_name . '.php';
